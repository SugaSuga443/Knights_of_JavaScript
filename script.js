...
function changePlayer() {
    ...
    if (gameState.whoseTurn === 1) {
        ...
        // generates a random number between the min and max damage values
        function getRandomDamage(minDamage,maxDamage){
            return Math.floor(Math.random() * (maxDamage - minDamage + 1) + minDamage); 
        };
        // reduces by a random number between 0 and 10
        playerTwoHealthNum -= getRandomDamage(0,10);
        ...
        if (playerTwoHealthNum <= 0) {
            playerTwoHealth.innerHTML = 0; //stops the health from being negative
            let winner = 1;
            gameOver(winner); //ends the game
        }
        else {
...
function gameOver(winnerID) {
    ...
    winningPlayer.innerHTML = `Player ${winnerID} wins!`;
